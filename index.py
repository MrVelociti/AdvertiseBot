...
random_servers_chnl = '599851356722823178'
console_chnl = '599851521764491277'
logs_chnl = '599851748798234655'
reports_users_chnl = '599853535378800643'
reports_servers_chnl = '599853662881185813'
reports_bugs_chnl = '599853990443745290'
suggestions_chnl = '599854483383517195'
banned_users_chnl = '599863901785554954'
banned_servers_chnl = '599863934148935680'
bot_mods_chnl = '599864114126520320'
bot_admins_chnl = '599864145466097664'
toggled_servers_chnl = '599864418620145675'
special_servers_chnl = '599864573129916426'
normal_servers_chnl = '599885593391923203'
channels_chnl = '599885701999362054'
log_channels_chnl = '599885812355694623'
normal_servers_msgs_chnl = '599885968673210378'
special_servers_msgs_chnl = '599886110524702740'
servers_links_chnl = '599886308458102785'
support_chnl = '599886555792015370'
warns_chnl = '599887259323334667'
mass_ban_chnl = '599887492946329601'
as_chnl = '599887527024918538'

support_server = '598100229052432396'
community_server = 'https://discord.gg'
owner_id = '402316460325601292'

loading_status = discord.Activity(name='- Loading...')
start_status = discord.Activity(name=' - *help | *support')
updating_status = discord.Activity(name=' - Updating...')
...
@client.event
async def on_ready():
    await client.change_presence(status=discord.Status.dnd, activity=loading_status)
    #await client.change_presence(game=discord.Activity(name=loading_status), status='dnd')
    ...
    msg += "\n+ Total server count: {}".format(len(client.guilds))
    ...
    await client.change_presence(status=discord.Status.online, activity=start_status)
    print("==========")
    print("==========")
    print("==========")
    print("BOT LOGGED IN")
        ...
        if server.id in ass:
            ...
            if user.id in banned_users:
                try:
                    ...
                    await client.send_message(client.get_channel('599887527024918538'), "{} | {} | 0".format(o, server.id))
...
async def help(ctx):
    ...
    try:
        ...
        await ctx.send("Check your DMs. :slight_smile: ")
    except:
        await ctx.send(":warning: Make sure the bot has permissions to send you DMs!")

# *ping
@client.command(pass_context=True)
async def ping(ctx):
    if ctx.message.guild.id in banned_servers:
        await ctx.send(":x: This server is in the ban list and cannot use the bot.")
    elif ctx.message.author.id in banned_users:
        await ctx.send(":x: You are on the ban list and cannot use the bot.")
    else:
    ...
    if ctx.message.guild.id in banned_servers:
        ...
        if args == None:
            ...
            if len(str(args)) > 1000:
                await ctx.send("{} The message cannot be longer than 1000 characters.".format(error_img))
            else:
                await ctx.send("Sending... <a:typing:393848431413559296>")
                msg = "@everyone "
                msg += "```diff"
                msg += "\n- SUPPORT -"
                ...
                await ctx.send("{} Message sent!".format(check_img))
                try:
                    ...
                    await ctx.send("{} Make sure the bot can send you DMs!".format(error_img))
...
async def info(ctx):
    if ctx.message.guild.id in banned_servers:
        ...
        big = []
        await ctx.send("Collecting information... <a:updating:403035325242540032>")
        msg = "```diff"
        ...
        if target == None:
            ...
            if len(found) == 0:
                ...
                try:
                    ...
                    await ctx.send(":warning: An unknown error occured while trying to collect the information for that server.")
...
async def suggest(ctx, *, args = None):
    if ctx.message.guild.id in banned_servers:
        ...
        server = ctx.message.guild
        if args == None:
            await ctx.send("{} No suggestion given.".format(error_img))
        else:
            if len(str(args)) > 1000:
                await ctx.send("{} The suggestion cannot be longer than 1000 characters.".format(error_img))
            else:
                await ctx.send("Sending suggestion... <a:typing:393848431413559296>")
                try:
                    ...
                    msg += "\n{}".format(args)
                    await client.send_message(chnl, msg)
                    await ctx.send(":white_check_mark: Suggestion sent!")
                except:
                    await ctx.send("{} Error in sending suggestion.".format(error_img))
            ...
            if option == "user" or option == "server":
                if target == None or reason == None:
                    ...
                    if len(str(reason)) > 1000:
                            await ctx.send("{} The reason cannot be longer than 1000 characters.".format(error_img))
                    else:
                        if option == "user":
                            found = []
                            chnl = client.get_channel(reports_users_chnl)
                            await ctx.send("Reporting... <a:typing:393848431413559296>")
                            try:
                                msg = "```diff"
                                msg += "\n- USER REPORT -"
                                msg += "\n+ Author: {} ### {}".format(author, author.id)
                                ...
                                msg += "\n{}".format(reason)
                                await client.send_message(chnl, msg)
                                await ctx.send("{} Reported!".format(check_img))
                            except:
                                await ctx.send("{} Error in sending report.".format(error_img))
                        elif option == "server":
                            found = []
    ...
    if ctx.message.guild.id in banned_servers:
        ...
        if args == None:
            ...
            if len(str(args)) > 1000:
                ...
                await ctx.send("Reporting the bug... <a:typing:393848431413559296>")
                try:
                    msg = "```diff"
                    msg += "\n- BUG REPORT -"
                    msg += "\n+ Author: {} ### {}".format(author, author.id)
                    ...
                    msg += "\n{}".format(args)
                    await client.send_message(chnl, msg)
                    await ctx.send("{} Reported the bug!".format(check_img))
                except:
                    await ctx.send("{} Error in reporting the bug!".format(error_img))
...
async def m(ctx, user: discord.User = None):
    ...
    if author.id in bot_admins:
        await ctx.send("{} <@{}> is a bot administrator.".format(check_img, author.id))
    elif author.id in bot_mods:
        await ctx.send("{} <@{}> is a bot moderator.".format(check_img, author.id))
    else:
        await ctx.send("{} <@{}> is not a bot administrator nor a bot moderator.".format(x_img, author.id))
        ...
        if server.id in ass:
            ...
            async for i in client.logs_from(chnl, limit=limit):
                if i.content == server.id:
                    ...
                    print("")
            await ctx.send("{} The auto-scan has been toggled on for this server.".format(check_img))
        else:
            ass.append(server.id)
            await client.send_message(chnl, server.id)
            await ctx.send("{} The auto-scan has been toggled off for this server.".format(check_img))
    else:
        await ctx.send("{} This command can only be used by users with the `Manage Server` permissions and can be bypassed by the bot's staff.".format(error_img))
    ...
    if ctx.message.guild.id in banned_servers:
        ...
        if author.server_permissions.manage_server or author.id in bot_mods or author.id in bot_admins:
            if channel == None or message == None or log_channel == None:
                ...
                await ctx.send("{} This server is already being advertised.\nYou can use `*unsetup` to un-set it up.".format(error_img))
            else:
                if len(str(message)) > 500:
                    await ctx.send("{} The message cannot be longer than 500 characters.".format(error_img))
                else:
                    urls = re.findall('http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+', message)
                    if len(urls) != 0:
                        await ctx.send("{} Please do not put any links in your message!".format(error_img))
                    else:
                        await ctx.send("Starting setup... <a:typing:393848431413559296>")
                        log = "```diff"
                        ...
                        try:
                            ...
                            for srv in client.guilds:
                                if srv.id == server.id:
                                    ...
                                    print("")
                            tl += "\n+ {}/{}".format(len(found), len(client.guilds))
                            tl += "\n```"
                            log += "\n+ Log message created!"
...
async def unsetup(ctx):
    ...
    if ctx.message.guild.id in banned_servers:
        ...
        if server.id in normal_servers:
            await ctx.send("Removing data... <a:typing:393848431413559296>")
            log = "```diff"
...
async def test(ctx):
    ...
    if ctx.message.guild.id in banned_servers:
        ...
        if server.id in normal_servers:
            await ctx.send("Testing... <a:typing:393848431413559296>")
            log = "```diff"
...
async def scan(ctx):
    ...
    if ctx.message.guild.id in banned_servers:
        ...
        await ctx.send("Scanning... <a:typing:393848431413559296>")
        ban = []
        fail = []
...
async def toggle(ctx):
    ...
    if ctx.message.guild.id in banned_servers:
        ...
        await ctx.send("Toggling... <a:typing:393848431413559296>")
        if server.id in toggled_servers:
            try:
                    ...
                    try:
                        ...
                        if log in server.channels:
                            try:
                                ...
                                await ctx.send("{} The bot will now continue advertising your server!\nThe toggle log was sent to <#{}>.".format(check_img, c))
                            except:
                                await ctx.send("{} The bot will now continue advertising your server!".format(check_img))
                            break
                        else:
                            print("")
                    except:
                ...
                await ctx.send("{} There has been an error in toggling this server!\nFor any help you can use `*support`.".format(error_img))
        else:
            try:
                await client.send_message(chnl, server.id)
                ...
                for c in log_channels_ids:
                    try:
                        ...
                        if log in server.channels:
                            try:
                                await client.send_message(log, msg)
                                await ctx.send("{} The bot won't advertise your server until you un-toggle it!\nThe toggle log was sent to <#{}>.".format(check_img, c))
                            except:
                                await ctx.send("{} The bot won't advertise your server until you un-toggle it!".format(check_img))
                        else:
                            print("")
                    except:
                        print("")
...
async def warn(ctx, option = None, target = None, *, reason = None):
    ...
    if author.id in bot_mods or author.id in bot_admins:
            ...
            if option == "user" or option == "server":
                async for i in client.logs_from(warnsc, limit=limit):
                    ...
                    if target == a[0]:
                        p.append("+1")
                        if len(p) == 3:
                            await ctx.send("That ID has already been warned 3 times. Use `*check <user/server> <id>` to check if they are banned.")
                            break
                        else:
                            if len(o) == 2:
                                break
                        ...
                        if option == "user":
                            try:
                                ...
                                try:
                                    ...
                                    await ctx.send("{} Unable to DM that user.".format(error_img))
                                banned_users.append(target)
                                await client.send_message(ubanc, "{} | Reached max warnings.".format(user.id))
                                await ctx.send("{} The user has been warned and banned!".format(check_img))
                            except:
                                await ctx.send("{} User not found!".format(error_img))
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    await ctx.send("{} Unable to DM the server owner.".format(error_img))
                                banned_servers.append(target)
                                await client.send_message(sbanc, "{} | Reached max warnings.".format(server.id))
                                await ctx.send("{} The server has been warned and banned!".format(check_img))
                            except:
                                await ctx.send("{} Server not found!".format(error_img))
                        ...
                        if option == "user":
                            try:
                                ...
                                try:
                                    ...
                                    await ctx.send("{} Unable to DM that user.".format(error_img))
                                await ctx.send("{} The user has been warned!".format(check_img))
                                await client.send_message(warnsc, "{} | {} ### {} | {} | user | {}".format(target, author, author.id, reason, len(p) + 1))
                            except:
                                await ctx.send("{} User not found!".format(error_img))
                        elif option == "server":
                            try:
                                ...
                                try:
                                    ...
                                    await ctx.send("{} Unable to DM the server owner.".format(error_img))
                                await ctx.send("{} The server has been warned!".format(check_img))
                            except:
                                await ctx.send("{} Server not found!".format(error_img))
                            await client.send_message(warnsc, "{} | {} ### {} | {} | server | {}".format(target, author, author.id, reason, len(p) + 1))
                        else:
        ...
        await ctx.send("{} This command can only be used by the bot's staff!".format(error_img))
    ...
    if author.id in bot_mods or author.id in bot_admins:
        if option == None or target == None or number == None:
            ...
            try:
                k = int(number)
                if k < 0 and k > 3:
                    await ctx.send("{} The number has to be 1, 2 or 3.")
                else:
                    if option == "user" or option == "server":
                        o = []
                        ...
                        async for i in client.logs_from(chnl, limit=limit):
                            ...
                            if target == a[0] and option == a[3] and number == a[4]:
                                await client.delete_message(i)
                                await ctx.send("{} Warning cleared!".format(check_img))
                                o.append("+1")
                                break
                            else:
                                print("")
                        if len(o) == 0:
                            await ctx.send("{} Warning not found!".format(error_img))
                        else:
                            print("")
                    else:
                        await ctx.send("{} Invalid option!\nOptions: `user`, `server`.".format(error_img))
            except:
                await ctx.send("{} That is not a number.".format(error_img))
    else:
        await ctx.send("{} This command can only be used by the bot's staff!".format(error_img))
...
async def remind(ctx):
    ...
    if author.id in bot_mods or author.id in bot_admins:
        o = []
        await ctx.send("Sending reminders...<a:typing:393848431413559296>")
        for c in log_channels_ids:
            try:
                ...
                m += "\n`-` Position: {}/{}".format(len(o), len(client.guilds))
                ...
                print("pass 2")
        await ctx.send("{} Finished!".format(check_img))
    else:
        await ctx.send("{} This command can only be used by the bot's staff!".format(error_img))
        ...
        if option == None or target == None or args == None:
            ...
            if option == "user" or option == "server":
                if len(str(args)) > 1900:
                    await ctx.send("{} The message cannot be longer than 1900 characters.".format(error_img))
                else:
                    if option == "user":
                        find = []
                        await ctx.send("Searching for user... <a:typing:393848431413559296>")
                        try:
                            user = await client.get_user_info(target)
                            ...
                            try:
                                ...
                                msg += "\n- USER MESSAGE -"
                                msg += "\n+ Author: {} ### {}".format(author, author.id)
                                msg += "\n+ From: {} ### {}".format(ctx.message.guild.name, ctx.message.guild.id)
                                ...
                                await ctx.send("{} Could not send the message to the user!\nEither the bot has no permission to send DMs to that user or they don't have a mutual server.".format(error_img))
                        except:
                            await ctx.send("{} User not found!".format(error_img))
                    elif option == "server":
                        find = []
                        finish = []
                        await ctx.send("Searching for server... <a:typing:393848431413559296>")
                        for server in client.guilds:
                            if server.id == target:
                                try:
                                    if len(finish) == 0:
        ...
        if option == None or target == None or reason == None:
            ...
            if option == "user" or option == "server":
                if len(str(reason)) > 1900:
                    await ctx.send("{} The reason cannot be longer than 1900 characters.".format(error_img))
                elif option == "user":
                    if target in banned_users:
                        await ctx.send("{} This user is already banned!".format(error_img))
                    else:
                        await client.send_message(chnl, "{} | {}".format(target, reason))
                        banned_users.append(target)
                        await ctx.send("{} **{}** has been added to the ban list!".format(check_img, target))
                        log = "```diff"
                        log += "\n- USER BAN -"
                        log += "\n+ Author: {} ### {}".format(author, author.id)
                        log += "\n+ From: {} ### {}".format(ctx.message.guild.name, ctx.message.guild.id)
                    ...
                    if target in banned_servers:
                        await ctx.send("{} This server is already banned!".format(error_img))
                    else:
            ...
            if option == "user":
                try:
                    ...
                    try:
                        ...
                        await ctx.send("{} The user has been unbanned!".format(check_img))
                        msg = "```diff"
                        msg += "\n- USER UNBAN -"
                        msg += "\n+ Author: {} ### {}".format(author, author.id)
                        msg += "\n+ From: {} ### {}".format(ctx.message.guild.name, ctx.message.guild.id)
                        ...
                        await ctx.send("{} That user is not banned!".format(error_img))
                except:
                    await ctx.send("{} There was an error while trying to unban that user!".format(error_img))
            elif option == "server":
                try:
                    ...
                    try:
                        ...
                        await ctx.send("{} The server has been unbanned!".format(check_img))
                        msg = "```diff"
                        msg += "\n- SERVER UNBAN -"
                        msg += "\n+ Author: {} ### {}".format(author, author.id)
                        msg += "\n+ From: {} ### {}".format(ctx.message.guild.name, ctx.message.guild.id)
                        ...
                        await ctx.send("{} That server is not banned!".format(error_img))
                except:
                    await ctx.send("{} There was an error while trying to unban that server!".format(error_img))
            else:
                await ctx.send("{} Invalid option!\nOptions: `user`, `server`.".format(error_img))
    else:
        await ctx.send("{} This command can only be used by the bot's staff!".format(error_img))
...
async def reset(ctx, target = None):
    ...
    if author.id in bot_mods or author.id in bot_admins:
        if target == None:
            await ctx.send("{} No server ID given!".format(error_img))
        else:
            ts = client.get_channel(toggled_servers_chnl)
            ss = client.get_channel(special_servers_chnl)
            ns = client.get_channel(normal_servers_chnl)
            ...
            try:
                ...
                if len(find) == 0:
                    ...
                    await ctx.send("{} Server found ( {} )!".format(check_img, server.name))
                    await ctx.send("Resetting the server... <a:typing:393848431413559296>")
                    try:
                        async for message in client.logs_from(ns, limit=limit):
...
async def link(ctx, target = None):
    ...
    if author.id in bot_mods or author.id in bot_admins:
        if target == None:
            ...
            for s in client.guilds:
                if s.id == target:
                    await ctx.send("{} Server found ( {} )!".format(check_img, s.name))
                    a.append("+1")
                    try:
                        invs = await client.invites_from(s)
                        await ctx.send("The server's link is: {}".format(invs[0]))
                        break
                    except:
                        await ctx.send("{} Unable to collect invites from that server.\nMake sure the bot has the required permissions in that server.".format(error_img))
                        break
                else:
                    print("")
            if len(a) == 0:
        ...
        if option == None or user == None:
            ...
            if option == "add":
                if user.id in bot_mods:
                    await ctx.send("{} That user is already a bot moderator!".format(error_img))
                else:
                    await client.send_message(bm, "{}".format(user.id))
                    bot_mods.append(user.id)
                    await ctx.send("{} Added {} to the bot moderators list.".format(check_img, user.name))
                    msg = "```diff"
                    msg += "\n- ADD MOD -"
                    msg += "\n+ Author: {} ### {}".format(author, author.id)
                    msg += "\n+ From: {} ### {}".format(ctx.message.guild.name, ctx.message.guild.id)
                ...
                if user.id in bot_mods:
                    ...
                    await ctx.send("{} Removed {} from the bot moderators list.".format(check_img, user.name))
                    msg = "```diff"
                    msg += "\n- DEL MOD -"
                    msg += "\n+ Author: {} ### {}".format(author, author.id)
                    msg += "\n+ From: {} ### {}".format(ctx.message.guild.name, ctx.message.guild.id)
                    ...
                    await ctx.send("{} That user is not a bot moderator!".format(error_img))
            else:
                await ctx.send("{} Invalid option!\nOptions: `add`, `del`.")
    else:
...
async def announce(ctx, *, args = None):
    ...
    if author.id in bot_admins:
        if args == None:
            ...
            await ctx.send("{} The text cannot be longer than 1900 characters.".format(error_img))
        else:
            await ctx.send("Sending announcement... <a:typing:393848431413559296>")
            done = []
            fail = []
            pos = []
            try:
                ...
                await ctx.send("{} There was an error while trying to send the announcements!".format(error_img))
    else:
        await ctx.send("{} This command can only be used by the bot's administrators.".format(error_img))
...
async def force(ctx):
    author = ctx.message.author
    if author.id in bot_admins:
        await ctx.send("Forcing advertisements... <a:typing:393848431413559296>")
        try:
            total = []
            normal = []
            special = []
...
async def log(ctx, *, args = None):
    ...
    if author.id in bot_admins:
        if args == None:
            ...
            await ctx.send("{} The arguments cannot be longer than 1500 characters.".format(error_img))
        else:
            try:
                args.split('|')
            except:
                await ctx.send("{} The command was used wrongly!\nExample: `*log SMALL UPDATE | The bot just got updated.`.".format(error_img))
            a = args.split('|')
            if len(a) != 2:
                await ctx.send("{} The command was used wrongly!\nExample: `*log SMALL UPDATE | The bot just got updated.`.".format(error_img))
            else:
                await ctx.send("Sending logs... <a:typing:393848431413559296>")
                done = []
                fail = []
                pos = []
                try:
                    ...
                    await ctx.send("{} There was an error while trying to send the logs!".format(error_img))
    else:
        await ctx.send("{} This command can only be used by the bot's administrators.".format(error_img))
    ...
    if author.id in bot_admins:
        if option == None or target == None:
            ...
            if target in special_servers:
                await ctx.send("{} That server is already in the special list.".format(error_img))
            else:
                try:
                    await ctx.send("Searching for the server... <a:typing:393848431413559296>")
                    ...
                    try:
                        if s.id in normal_servers:
                            ...
                            if len(b) == 0:
                                await ctx.send(log)
                                await ctx.send("{} The server should now have special advertisements!".format(check_img))
                            else:
                                await ctx.send("{} The server should now have special advertisements!\nYou can find the log in <#{}>.".format(check_img, b[0]))
                            o = "```diff"
                            o += "\n- ADD SPECIAL -"
                            o += "\n+ Author: {} ### {}".format(author, author.id)
                            o += "\n+ From: {} ### {}".format(ctx.message.guild.name, ctx.message.guild.id)
                            ...
                            await ctx.send("{} That server isn't setup.\nTry again once the server has been setup.".format(error_img))
                    except:
                        log += "\n- ^ Error!"
                        log += "\n--- CLOSING SPECIAL LOG ---"
                        log += "\n```"
                        await ctx.send(log)
                        await ctx.send("{} There was an error while trying to add the server to the special list.".format(error_img))
                except:
            ...
            try:
                ...
                try:
                    if s.id in special_servers:
                        ...
                        if len(b) == 0:
                            await ctx.send(log)
                            await ctx.send("{} The server has been removed from the special list!".format(check_img))
                        else:
                            await ctx.send("{} The server has been removed from the special list!\nYou can find the special log in <#{}>.".format(check_img, b[0]))
                        o = "```diff"
                        o += "\n- DEL SPECIAL -"
                        o += "\n+ Author: {} ### {}".format(author, author.id)
                        o += "\n+ From: {} ### {}".format(ctx.message.guild.name, ctx.message.guild.id)
                        ...
                        await ctx.send("{} The server is not in the special list.".format(error_img))
                except:
                    log += "\n- ^ Error!"
                    log += "\n--- CLOSING SPECIAL LOG ---"
                    log += "\n```"
                    await ctx.send(log)
                    await ctx.send("{} There was an error while trying to remove the server from the special list.".format(error_img))
            except:
                await ctx.send("{} Server not found!\nMake sure the bot is in that server and it has the required permissions.".format(error_img))
        else:
...
async def say(ctx, *, args = None):
    ...
    if author.id in bot_admins:
        if args == None:
            await ctx.send("{} No text given!".format(error_img))
        else:
            if len(str(args)) > 1900:
                await ctx.send("{} The text cannot be longer than 1900 characters.".format(error_img))
            else:
        ...
        if option == None:
            ...
            if option == "clear":
                try:
                    ...
                    await ctx.send("{} Cleared!".format(check_img))
                except:
                    await ctx.send("{} Failed to clear!".format(check_img))
            elif option == "run":
                if args == None:
                    ...
                    reason = args
                o = []
                await ctx.send("Saving... <a:typing:393848431413559296>")
                async for i in client.logs_from(chnl, limit=limit):
                    a = i.content.split('\n')
                    for k in a:
                        o.append(k)
...
async def ms(ctx):
    ...
    if author.id in bot_admins:
        if len(mss) == 0:
            await ctx.send("Saving servers... <a:typing:393848431413559296>")
            mss.append("+1")
            u = []
            ...
            h = await ctx.send("This message will be updated with more information until the mass scan finishes.")
            hh = []
            for i in u:
                try:
            ...
            m += "\n+ Author: {} ### {}".format(author, author.id)
            m += "\n+ From: {} ### {}".format(ctx.message.guild.name, ctx.message.guild.id)
            m += "\n+ Servers passed: {}/{}".format(len(hh), len(client.guilds))
            ...
            await ctx.send("{} A mass scan is already running.".format(error_img))
    else:
        await ctx.send("{} This command can only be used by the bot's administrators.".format(error_img))
